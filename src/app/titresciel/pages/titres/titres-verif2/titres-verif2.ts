...
import { Subscription, Subject, map, catchError, of, finalize, concatMap, forkJoin, switchMap, fromEvent } from 'rxjs';
...
@Component({
  selector: 'app-titres-verif2',
  standalone: false,
  templateUrl: './titres-verif2.html',
  styleUrl: './titres-verif2.scss'
})
export class TitresVerif2 implements OnInit, AfterViewInit, OnDestroy{
  submitting = false;
  submitted = false;
  downloaded=false
  confirmer=false
  ...
  verificationCours(){
    ...
    const params = {
        ...
        a.href = url;
        a.download = 'verification_cours_titre_niveau2.pdf';
        a.click();
      });
  }
  validerVerificationCours(){
    ...
    const params = {
        ...
        finalize(() => {
          //this.downloading = false;
          this.submitting = false;
          alert("Confirmation de la vérification de la mise à jour des cours niveau 2 effectuée avec succès")
