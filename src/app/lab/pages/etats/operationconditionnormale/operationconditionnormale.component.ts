...
export class OperationconditionnormaleComponent implements OnInit, OnDestroy, AfterViewInit{
  ...
  ngOnInit(): void {
    ...
    this.pageInfo.updateTitle("Point des transactions personnes physiques supérieures ou " +
      "égales à 50 000 000 effectuées dans les conditions normales sur l'année")
    // this.afficherDepot();
  }
  ...
  afficherOperation(){
    ...
    this.operationService.afficherTransactionNormale(this.annee).subscribe(
      (data)=>{
...
<!--          </div>-->
  ...
  generateExcelTable() {
    ...
    let name="opération-effectuée-condition-normale"
    const timeSpan = new Date().toISOString();
    const fileName = `${name}`;
    const targetTableElm = document.getElementById("reporting-condition-normale");
    const wb = XLSX.utils.table_to_book(targetTableElm, { sheet: "sheet1" } as
      XLSX.Table2SheetOpts);
    XLSX.writeFile(wb, `${fileName}.xlsx`);
  }
  async downloadPdf() {
    var data = document.getElementById("reporting-condition-normale");
    // $("pdfOpenHide").attr("hidden", true);
    // To disable the scroll
    // @ts-ignore
    document.getElementById("alldata-condition-normale").style.overflow = "inherit";
    // @ts-ignore
    document.getElementById("alldata-condition-normale").style.maxHeight = "inherit";
    ...
    await html2canvas(data, { scrollY: -window.scrollY, scale: 1 }).then(
      canvas => {
        ...
        // enabling the scroll
        // @ts-ignore
        document.getElementById("alldata-condition-normale").style.overflow = "scroll";
        // @ts-ignore
        document.getElementById("alldata-condition-normale").style.maxHeight = "150px";
        ...
        pdf.save('opération-effectuée-condition-normale.pdf');
        // window.open(
