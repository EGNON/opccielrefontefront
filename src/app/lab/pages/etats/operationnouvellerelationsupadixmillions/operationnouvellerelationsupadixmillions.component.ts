...
export class OperationnouvellerelationsupadixmillionsComponent implements OnInit, OnDestroy, AfterViewInit{
  ...
  ngOnInit(): void {
    ...
    this.pageInfo.updateTitle("Point des transactions personnes physiques supérieures ou égales à 10 000 000 constituants des nouvelles relations d'affaires sur l'année")
    // this.afficherDepot();
  }
...
//   }
  generateExcelTable() {
    ...
    let name="opération-constituant-de-nouvelle-relation"
    const timeSpan = new Date().toISOString();
    const fileName = `${name}`;
    const targetTableElm = document.getElementById("reporting-nouvelle-relation");
