...
@Component({
  selector: 'app-personne-morale-expose-add-edit',
  templateUrl: './personne-morale-expose-add-edit.component.html',
  styleUrl: './personne-morale-expose-add-edit.component.scss'
})
export class PersonneMoraleExposeAddEditComponent implements OnInit, OnDestroy, AfterViewInit{
  public paysSettings = {};
  pays: any;
  qualite: string;
  id?: number;
  ...
  ngOnInit() {
    ...
    this.personneSelect=document.getElementById("ComboPersonneMoraleExistanteExpose")
      ...
      const sb = this.saveEntity(formdata)
        .pipe(
          ...
          finalize(() => {
            ...
            this.router.navigate([`/lab/standard/parametre/personne/morale/politiquementexpose`]);
          })
        )
        .subscribe(res => {
    ...
    else
    {
      ...
      this.personneMoraleService.afficherPersonneMoraleSelonId(idPersonne).subscribe(
        (data)=>{
          ...
          // this.personneMorale.estExpose=this.entityForm.get('estExpose').checked;

          this.personneMorale.estExpose = true;
