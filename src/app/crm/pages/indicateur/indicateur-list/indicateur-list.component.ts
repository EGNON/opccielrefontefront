...
export class IndicateurListComponent implements OnInit, OnDestroy, AfterViewInit{
  ...
  constructor(
    ...
    public indicateurService: IndicateurService,
  ...
  afficherIndicateur(){
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        const sb = this.indicateurService.afficherTous(dataTablesParameters)
          .subscribe(resp => {
            callback(resp);
          });
      ...
      columns: [
        ...
        {
          title: 'Indicateur', data: 'libelle', render: function (data, type, row) {
            return row.libelle;
          },
        }
      ],
  ...
  renderActionColumn(): void {
    if (this.datatableConfig.columns) {
      ...
      actions.render = (data: any, type: any, full: any) => {
        ...
        const show = `
                <li>
                    <a type="button" class="dropdown-item" data-action="view" data-id="${full.idIndicateur}">Afficher</a>
                </li>`;
        const edit = `
                <li>
                    <a type="button" class="dropdown-item" data-action="edit" data-id="${full.idIndicateur}">Modifier</a>
                </li>`;
        const separator = `<li><hr class="dropdown-divider"></li>`;
        const delete1 = `<li>
                    <a type="button" class="dropdown-item" data-action="delete" data-id="${full.idIndicateur}">Supprimer</a>
                </li>`;
