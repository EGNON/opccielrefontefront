...
export class ModeetablissementListComponent  implements OnInit, OnDestroy, AfterViewInit{
  ...
  constructor(
    ...
    public modeetablissementService: ModeetablissementService,
    private router: Router,
    private modalService: NgbModal) {}

  ngOnInit(): void {
    this.afficherModeEtablissement();
  }
  ...
  afficherModeEtablissement(){
    this.datatableConfig = {
      serverSide: true,
      ajax: (dataTablesParameters: any, callback) => {
        const sb = this.modeetablissementService.afficherTous(dataTablesParameters)
          .subscribe(resp => {
            callback(resp);
          });
  ...
  renderActionColumn(): void {
    if (this.datatableConfig.columns) {
      ...
      actions.render = (data: any, type: any, full: any) => {
        ...
        const show = `
                <li>
                    <a type="button" class="dropdown-item" data-action="view" data-id="${full.idModeEtablissement}">Afficher</a>
                </li>`;
        const edit = `
                <li>
                    <a type="button" class="dropdown-item" data-action="edit" data-id="${full.idModeEtablissement}">Modifier</a>
                </li>`;
        const separator = `<li><hr class="dropdown-divider"></li>`;
        const delete1 = `<li>
                    <a type="button" class="dropdown-item" data-action="delete" data-id="${full.idModeEtablissement}">Supprimer</a>
                </li>`;
