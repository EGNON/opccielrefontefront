...
export class TypedocumentListComponent implements OnInit, OnDestroy, AfterViewInit{
  ...
  afficherTypeDocument(){
    this.datatableConfig = {
      serverSide: true,
      ajax: (dataTablesParameters: any, callback) => {
        const sb = this.typeDocumentService.afficherTousDataTable(dataTablesParameters)
          .subscribe(resp => {
            callback(resp);
          });
      ...
      columns: [
        {
          title: 'Libellé', data: 'libelleTypeDoc', render: function (data, type, row) {
            return row.libelleTypeDoc;
          },
        },
      ],
  ...
  renderActionColumn(): void {
    if (this.datatableConfig.columns) {
      ...
      actions.render = (data: any, type: any, full: any) => {
        ...
        const show = `
                <li>
                    <a type="button" class="dropdown-item" data-action="view" data-id="${full.idTypeDoc}">Afficher</a>
                </li>`;
        const edit = `
                <li>
                    <a type="button" class="dropdown-item" data-action="edit" data-id="${full.idTypeDoc}">Modifier</a>
                </li>`;
        const separator = `<li><hr class="dropdown-divider"></li>`;
        const delete1 = `<li>
                    <a type="button" class="dropdown-item" data-action="delete" data-id="${full.idTypeDoc}">Supprimer</a>
                </li>`;
...
//OnInit, OnDestroy{
//   isLoading: boolean;
//   entities$: Observable<Typedocument[]>;
//   private subscriptions: Subscription[] = [];
//
//   constructor(
