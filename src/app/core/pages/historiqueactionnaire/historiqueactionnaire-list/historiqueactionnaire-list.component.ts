...
export class HistoriqueactionnaireListComponent implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  ...
  exportExcel() {
    ...
    this.libService.historiqueActionnaireListe(param).subscribe(
      (data)=>{
  ...
  telecharger() {
    ...
    param = {
      ...param,
      startDate: new Date(param.startDate.year, param.startDate.month - 1, param.startDate.day + 1),
      endDate: new Date(param.endDate.year, param.endDate.month - 1, param.endDate.day + 1),
    }
    //.subscribe
    const sb = this.libService.historiqueActionnaireEtat(param)
      .pipe(
        catchError((err) => {
      ...
      .subscribe((blob) => {
        ...
        a.href = url;
        a.download = 'historique_actionnaire.pdf';
        a.click();
      });
    this.subscriptions.push(sb);
  }
