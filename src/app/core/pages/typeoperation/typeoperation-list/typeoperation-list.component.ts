...
export class TypeoperationListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    public entityService: TypeoperationService,
  ...
  ngOnInit(): void {
    this.datatableConfig = {
      ...
      columns: [
        {
          title: 'Code', data: 'codeTypeOperation', render: function (data, type, row) {
            return row.codeTypeOperation;
          }
        },{
          title: 'Libellé', data: 'libelleTypeOperation', render: function (data, type, row) {
            return row.libelleTypeOperation;
          }
        },
      ],
  ...
  renderOperationColumn(): void {
    if (this.datatableConfig.columns) {
      ...
      Operations.render = (data: any, type: any, full: any) => {
        ...
        const show = `
                <li>
                    <a type="button" class="dropdown-item" data-action="view" data-id="${full.codeTypeOperation}">Afficher</a>
                </li>`;
        const edit = `
                <li>
                    <a type="button" class="dropdown-item" data-action="edit" data-id="${full.codeTypeOperation}">Modifier</a>
                </li>`;
        const separator = `<li><hr class="dropdown-divider"></li>`;
        const delete1 = `<li>
                    <a type="button" class="dropdown-item" data-action="delete" data-id="${full.codeTypeOperation}">Supprimer</a>
                </li>`;
  ...
  supprimer(id: any) {
    const modalRef = this.modalService.open(DeleteTypeoperationModalComponent);
