...
export class TypeoperationAddEditComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private entityService: TypeoperationService,
  ...
  ngOnInit(): void {
    ...
    this.entityForm = this.fb.group(
      {
        id: [null],
        libelleTypeOperation: [null, Validators.required],
        codeTypeOperation: [null, Validators.required],
      }
    );
    this.readOnly=false;
    ...
    if (this.id) {
      this.readOnly=true;
      console.log(this.id)
      this.pageInfo.updateTitle("Modification de type opération")
      ...
      this.pageInfo.updateTitle("Ajout de type opération")
    }
  }
  loadFormValues(entity: any)
  {
    this.entity = entity;
    this.entityForm.patchValue({libelleTypeOperation: entity.libelleTypeOperation});
    this.entityForm.patchValue({codeTypeOperation: entity.codeTypeOperation});
  }
  ...
  onSaveEntity() {
    ...
    const sb = this.saveEntity().pipe(
      ...
      finalize(() => {
        ...
        if (!this.isModal) this.router.navigate(['/app/standard/parametre/comptabilite/typeoperation']);
      })
    )
      .subscribe();
    this.subscriptions.push(sb);
