...
export class TypeemissionAddEditComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private entityService: TypeemissionService,
  ...
  ngOnInit(): void {
    ...
    this.entityForm = this.fb.group(
      {
        id: [null],
        libelleTypeEmission: [null, Validators.required],
      }
    );
    // this.afficherCategorie();
    if (this.id) {
      console.log(this.id)
      this.pageInfo.updateTitle("Modification de type emission")
      ...
      this.pageInfo.updateTitle("Ajout de type emission")
    }
  }
  loadFormValues(entity: any)
  {
    this.entity = entity;
    this.entityForm.patchValue({libelleTypeEmission: entity.libelleTypeEmission});
  }
  ...
  onSaveEntity() {
    ...
    const sb = this.saveEntity().pipe(
      ...
      finalize(() => {
        ...
        if (!this.isModal) this.router.navigate(['/app/standard/parametre/instrumentfinancier/typeemission']);
      })
    )
      .subscribe();
    this.subscriptions.push(sb);
