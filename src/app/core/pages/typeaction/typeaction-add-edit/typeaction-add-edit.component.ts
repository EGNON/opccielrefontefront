...
export class TypeactionAddEditComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.entityForm = this.fb.group(
      {
        id: [null],
        libelleTypeAction: [null, Validators.required],
        codeTypeAction: [null, Validators.required],
      }
    );
    // this.afficherCategorie();
    if (this.id) {
      console.log(this.id)
      this.pageInfo.updateTitle("Modification de type action")
      ...
      this.pageInfo.updateTitle("Ajout de type action")
    }
  }
  loadFormValues(entity: any)
  {
    this.entity = entity;
    this.entityForm.patchValue({libelleTypeAction: entity.libelleTypeAction});
    this.entityForm.patchValue({codeTypeAction: entity.codeTypeAction});
  }
  ...
  onSaveEntity() {
    ...
    const sb = this.saveEntity().pipe(
      ...
      finalize(() => {
        ...
        if (!this.isModal) this.router.navigate(['/app/standard/parametre/instrumentfinancier/typeaction']);
      })
    )
      .subscribe();
    this.subscriptions.push(sb);
