...
export class DegreListComponent implements OnInit, OnDestroy, AfterViewInit{
  ...
  constructor(
    ...
    public entityService: DegreService,
  ...
  renderActionColumn(): void {
    if (this.datatableConfig.columns) {
      ...
      actions.render = (data: any, type: any, full: any) => {
        ...
        const show = `
                <li>
                    <a type="button" class="dropdown-item" data-action="view" data-id="${full.idDegre}">Afficher</a>
                </li>`;
        const edit = `
                <li>
                    <a type="button" class="dropdown-item" data-action="edit" data-id="${full.idDegre}">Modifier</a>
                </li>`;
        const separator = `<li><hr class="dropdown-divider"></li>`;
        const delete1 = `<li>
                    <a type="button" class="dropdown-item" data-action="delete" data-id="${full.idDegre}">Supprimer</a>
                </li>`;
  ...
  supprimer(id: number) {
    const modalRef = this.modalService.open(DeleteDegreModalComponent);
