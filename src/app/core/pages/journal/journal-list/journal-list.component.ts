...
export class JournalListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    public entityService: JournalService,
  ...
  ngOnInit(): void {
    this.datatableConfig = {
      ...
      columns: [
        {
          title: 'Code', data: 'codeJournal', render: function (data, type, row) {
            return row.codeJournal;
          }
        },{
          title: 'Libellé', data: 'libelleJournal', render: function (data, type, row) {
            return row.libelleJournal;
          }
        },
      ],
  ...
  renderOperationColumn(): void {
    if (this.datatableConfig.columns) {
      ...
      Operations.render = (data: any, type: any, full: any) => {
        ...
        const show = `
                <li>
                    <a type="button" class="dropdown-item" data-action="view" data-id="${full.codeJournal}">Afficher</a>
                </li>`;
        const edit = `
                <li>
                    <a type="button" class="dropdown-item" data-action="edit" data-id="${full.codeJournal}">Modifier</a>
                </li>`;
        const separator = `<li><hr class="dropdown-divider"></li>`;
        const delete1 = `<li>
                    <a type="button" class="dropdown-item" data-action="delete" data-id="${full.codeJournal}">Supprimer</a>
                </li>`;
  ...
  supprimer(id: any) {
    const modalRef = this.modalService.open(DeleteJournalModalComponent);
