...
export class FicheclientComponent implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  ...
  afficherActionnaire(){
    if(this.form.value.estPH===true)
      this.typePersonne="PH"
    else
      this.typePersonne="PM"

    this.libService.afficherPersonnePhysiqueMoraleSelonType(this.typePersonne).subscribe(
      (data)=>{
  ...
  rechercherActionnaire(){
    ...
    if(valeur!==undefined && valeur!==null)
      this.libService.rechercherPersonnePhysiqueMoraleSelonType(this.typePersonne,valeur).subscribe(
        (data)=>{
          this.actionnaire$=data.data
        }
      )
  ...
  telecharger() {
    ...
    //.subscribe
    if(this.typePersonne==="PH")
      this.estPH=true
    else
      this.estPH=false

    const sb = this.libService.ficheClientEtat(param)
      .pipe(
        catchError((err) => {
