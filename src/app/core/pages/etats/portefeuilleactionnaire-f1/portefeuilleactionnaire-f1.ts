...
@Component({
  selector: 'app-portefeuilleactionnaire-f1',
  standalone: false,
  templateUrl: './portefeuilleactionnaire-f1.html',
  styleUrl: './portefeuilleactionnaire-f1.scss'
})
export class PortefeuilleactionnaireF1 implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  form: FormGroup;
  ...
  telecharger() {
    ...
    //.subscribe
    const sb = this.libService.portefeuilleActionnaire(param)
      .pipe(
        catchError((err) => {
     ...
     .subscribe((blob) => {
        ...
        a.href = url;
        a.download = 'portefeuille_actionnaire_F1.pdf';
        a.click();
      });
    this.subscriptions.push(sb);
  }
