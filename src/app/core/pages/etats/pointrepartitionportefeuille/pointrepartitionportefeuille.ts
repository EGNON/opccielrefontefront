...
@Component({
  selector: 'app-pointrepartitionportefeuille',
  standalone: false,
  templateUrl: './pointrepartitionportefeuille.html',
  styleUrl: './pointrepartitionportefeuille.scss'
})
export class Pointrepartitionportefeuille implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  form: FormGroup;
  ...
  afficherListe(prefix: string) {
    ...
    let columns: any[] = [
      ...
      {
        title: 'ACTION (Montant)', data: 'actionsMontant', render: function (data, type, row) {
          return row.actionsMontant
        },
      },
      {
        title: 'Action (%)', data: 'actionsPC', render: function (data, type, row) {
          return row.actionsPC;
        },
      },
      {
        title: 'OBLIGATION COTEE (Montant)', data: 'obligationCoteMontant', render: function (data, type, row) {
          return row.obligationCoteMontant;
        },
      },
      {
        title: 'OBLIGATION COTEE (%)', data: 'obligationCotePC', render: function (data, type, row) {
          return row.obligationCotePC;
        },
      },
      {
        title: 'OBLIGATION NON COTEE (Montant)', data: 'obligationNcoteMontant', render: function (data, type, row) {
          return row.obligationNcoteMontant;
        },
      },
      {
        title: 'OBLIGATION NON COTEE (%)', data: 'obligationNcoteMontantPC', render: function (data, type, row) {
          return row.obligationNcotePC;
        },
      },
      {
        title: 'BON DE TRESOR (Montant)', data: 'botMontant', render: function (data, type, row) {
          return row.botMontant;
        },
      },
      {
        title: 'BON DE TRESOR (%)', data: 'botMontantPC', render: function (data, type, row) {
          return row.botPC;
        },
      },
      {
        title: 'LIQUIDITE (Montant)', data: 'soldeEspece', render: function (data, type, row) {
          return row.soldeEspece;
        },
      },
      {
        title: 'LIQUIDITE (%)', data: 'soldeEspecePC', render: function (data, type, row) {
          return row.soldeEspecePC;
        },
      },
    ...
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if (prefix.toLowerCase() === "l") {
          ...
          console.log(param);
          const sb = this.libService.pointRepartitionPortefeuille(param)
            .subscribe(resp => {
  ...
  let columns: any[] = [
      ...
      {
        title: 'OBLIGATION NON COTEE (%)', data: 'obligationNcoteMontantPC', render: function (data, type, row) {
          return row.obligationNcoteMontantPC;
        },
      },
      {
        title: 'BON DE TRESOR (Montant)', data: 'botMontant', render: function (data, type, row) {
    ...
    const headers = ['DESIGNATION','ACTION (Montant)','Action (%)',
      'OBLIGATION COTEE (Montant)','OBLIGATION COTEE (%)','OBLIGATION NON COTEE (Montant)'
    ,'OBLIGATION NON COTEE (%)','BON DE TRESOR (Montant)','BON DE TRESOR (%)','LIQUIDITE (Montant)'
  ,'LIQUIDITE (%)'];
    let idOpcvm = this.currentOpcvm?.idOpcvm;
    let param = {
      idOpcvm: idOpcvm,
    ...
    this.libService.pointRepartitionPortefeuilleListe(param).subscribe(
      (data)=>{
        this.allData=data.data;
        const exportData = this.allData.map(item => ({
          'DESIGNATION': item.denominationOpcvm,
          'ACTION (Montant)':item.actionsMontant,
          'Action (%)': item.actionsPC,
          'OBLIGATION COTEE (Montant)': item.obligationCoteMontant,
          'OBLIGATION COTEE (%)': item.obligationCotePC,
          'OBLIGATION NON COTEE (Montant)': item.obligationNcoteMontant,
          'OBLIGATION NON COTEE (%)': item.obligationNcotePC,
          'BON DE TRESOR (Montant)': item.botMontant,
          'BON DE TRESOR (%)': item.botPC,
          'LIQUIDITE (Montant)': item.soldeEspece,
          'LIQUIDITE (%)': item.soldeEspecePC,
        ...
        saveAs(blob, 'pointRepartitionPortefeuille.xlsx');
        this.export=false
      }
    )
  ...
  telecharger() {
    ...
    //.subscribe
    const sb = this.libService.pointRepartitionPortefeuilleEtat(param)
      .pipe(
        catchError((err) => {
      ...
      .subscribe((blob) => {
        ...
        a.href = url;
        a.download = 'point_repartition_portefeuille.pdf';
        a.click();
      });
    this.subscriptions.push(sb);
  }
