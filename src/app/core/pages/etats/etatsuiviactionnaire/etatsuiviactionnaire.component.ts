...
@Component({
    selector: 'app-etatsuiviactionnaire',
    templateUrl: './etatsuiviactionnaire.component.html',
    styleUrl: './etatsuiviactionnaire.component.scss',
    standalone: false
})
export class EtatsuiviactionnaireComponent implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  form: FormGroup;
  ...
  afficherListe(prefix: string) {
    ...
    let columns: any[] = [
      ...
      {
        title: 'NBRE. ACT. INI.', data: 'nbreInitial', render: function (data, type, row) {
          return row.nbreInitial
        },
      },
      {
        title: 'NBRE. PATRS. INI.', data: 'partInitial', render: function (data, type, row) {
          return row.partInitial;
        },
      },
      {
        title: 'NBRE. ACT. ENTR.', data: 'nbreEntrantAnc', render: function (data, type, row) {
          return row.nbreEntrantAnc;
        },
      },
      {
        title: 'NBRE. PART. ENTR.', data: 'partEntrant', render: function (data, type, row) {
          return row.partEntrant;
        },
      },
      {
        title: 'NBRE. ACT. SORT.', data: 'nbreSortantAnc', render: function (data, type, row) {
          return row.nbreSortantAnc;
        },
      },
      {
        title: 'NBRE. PART. SORT.', data: 'partSortant', render: function (data, type, row) {
          return row.partSortant;
        },
      },
      {
        title: 'NBRE. ACT. FIN', data: 'nbreFinal', render: function (data, type, row) {
          return row.nbreFinal;
        },
      },
      {
        title: 'NBRE. PART. FIN.', data: 'partFinal', render: function (data, type, row) {
          return row.partFinal;
        },
      },
      // {
    ...
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if (prefix.toLowerCase() === "l") {
          ...
          console.log(param);
          const sb = this.libService.etatSuiviAtionnaire(param)
            .subscribe(resp => {
  ...
  exportExcel() {
    ...
    const headers = ['DENOMINATION','NBRE. ACT. INI.','NBRE. PATRS. INI.','NBRE. ACT. ENTR.',
      'NBRE. PART. ENTR.','NBRE. ACT. SORT.','NBRE. PART. SORT.','NBRE. ACT. FIN','NBRE. PART. FIN.'];
    let idOpcvm = this.currentOpcvm?.idOpcvm;
    let param = {
      idOpcvm: idOpcvm,
      ...this.form.value,
    ...
    this.libService.etatSuiviAtionnaireListe(param).subscribe(
      (data)=>{
        this.allData=data.data;
        const exportData = this.allData.map(item => ({
          'DENOMINATION': item.denominationOpcvm,
          'NBRE. ACT. INI.':item.nbreInitial,
          'NBRE. PATRS. INI.': item.partInitial,
          'NBRE. ACT. ENTR.': item.nbreEntrantAnc,
          'NBRE. PART. ENTR.': item.partEntrant,
          'NBRE. ACT. SORT.': item.nbreSortantAnc,
          'NBRE. PART. SORT.': item.partSortant,
          'NBRE. ACT. FIN': item.nbreFinal,
          'NBRE. PART. FIN.': item.partFinal,
        }));
        ...
        saveAs(blob, 'historiqueActionnaire.xlsx');
        this.export=false
      }
    )
  ...
  telecharger() {
    ...
    //.subscribe
    const sb = this.libService.etatSuiviAtionnaireEtat(param)
      .pipe(
        catchError((err) => {
      ...
      .subscribe((blob) => {
        ...
        a.href = url;
        a.download = 'etat_suivi_actionnaire.pdf';
        a.click();
      });
    this.subscriptions.push(sb);
  }
