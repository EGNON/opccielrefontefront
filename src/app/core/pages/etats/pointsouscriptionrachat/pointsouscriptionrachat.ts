...
@Component({
  selector: 'app-pointsouscriptionrachat',
  standalone: false,
  templateUrl: './pointsouscriptionrachat.html',
  styleUrl: './pointsouscriptionrachat.scss'
})
export class Pointsouscriptionrachat implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  form: FormGroup;
  ...
  afficherListe(prefix: string) {
    ...
    let columns: any[] = [
      {
        title: 'DENOMINATION', data: 'designationOpcvm', render: function (data, type, row) {
          return row.designationOpcvm
        },
      },
      {
        title: 'PERSONNE PHYSIQUE', data: 'pp', render: function (data, type, row) {
          return row.pp
        },
      },
      {
        title: 'NB. PART PP', data: 'partpp', render: function (data, type, row) {
          return row.partpp;
        },
      },
      {
        title: 'PERSONNE MORALE', data: 'pm', render: function (data, type, row) {
          return row.pm;
        },
      },
      {
        title: 'NB. PART PP', data: 'partpm', render: function (data, type, row) {
          return row.partpm;
        },
      },
      {
        title: 'COMMISSION', data: 'com', render: function (data, type, row) {
          return row.com;
        },
      },
    ...
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if (prefix.toLowerCase() === "l") {
          ...
          console.log(param);
          const sb = this.libService.pointSousriptionRachat(param)
            .subscribe(resp => {
  ...
  let columns: any[] = [
      ...
      {
        title: 'NB. PART PM', data: 'partpm', render: function (data, type, row) {
          return row.partpm;
        },
      },
      {
    ...
    const headers = ['DENOMINATION','PERSONNE PHYSIQUE','NB. PART PP',
      'PERSONNE MORALE','NB. PART PM','COMMISSION'];
    let idOpcvm = this.currentOpcvm?.idOpcvm;
    let param = {
      idOpcvm: idOpcvm,
      ...this.form.value,
    ...
    this.libService.pointSousriptionRachatListe(param).subscribe(
      (data)=>{
        this.allData=data.data;
        const exportData = this.allData.map(item => ({
          'DENOMINATION': item.designationOpcvm,
          'PERSONNE PHYSIQUE':item.pp,
          'NB. PART PP': item.partpp,
          'PERSONNE MORALE': item.pm,
          'NB. PART PM': item.partpm,
          'COMMISSION': item.com,
        ...
        saveAs(blob, 'pointsouscription.xlsx');
        this.export=false
      }
    )
  ...
  telecharger() {
    ...
    //.subscribe
    const sb = this.libService.pointSouscriptionEtat(param)
      .pipe(
        catchError((err) => {
     ...
     .subscribe((blob) => {
        ...
        a.href = url;
        a.download = 'point_souscription.pdf';
        a.click();
      });
    this.subscriptions.push(sb);
  }
