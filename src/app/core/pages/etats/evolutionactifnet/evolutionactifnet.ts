...
@Component({
  selector: 'app-evolutionactifnet',
  standalone: false,
  templateUrl: './evolutionactifnet.html',
  styleUrl: './evolutionactifnet.scss'
})
export class Evolutionactifnet implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  form: FormGroup;
  ...
  telecharger() {
    ...
    //.subscribe
    const sb = this.libService.evolutionActifNetEtat(param)
      .pipe(
        catchError((err) => {
     ...
     .subscribe((blob) => {
        ...
        a.href = url;
        a.download = 'evolution_actif_net.pdf';
        a.click();
      });
    this.subscriptions.push(sb);
  }
