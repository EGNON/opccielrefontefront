...
export class TypeclientAddEditComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private entityService: TypeclientService,
  ...
  ngOnInit(): void {
    ...
    if (this.id) {
      this.readOnly=true;
      console.log(this.id)
      this.pageInfo.updateTitle("Modification de Type Client")
      ...
      this.pageInfo.updateTitle("Ajout de Type Client")
    }
  }
  ...
  onSaveEntity() {
    const sb = this.saveEntity().pipe(
      ...
      finalize(() => {
        ...
        if (!this.isModal) this.router.navigate(['/app/standard/parametre/nomenclature/typeclient']);
      })
    )
      .subscribe();
    this.subscriptions.push(sb);
