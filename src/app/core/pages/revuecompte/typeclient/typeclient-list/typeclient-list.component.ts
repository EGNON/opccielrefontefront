...
export class TypeclientListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    public entityService: TypeclientService,
  ...
  renderOperationColumn(): void {
    if (this.datatableConfig.columns) {
      ...
      Operations.render = (data: any, type: any, full: any) => {
        ...
        const show = `
                <li>
                    <a type="button" class="dropdown-item" data-action="view" data-id="${full.idTypeClient}">Afficher</a>
                </li>`;
        const edit = `
                <li>
                    <a type="button" class="dropdown-item" data-action="edit" data-id="${full.idTypeClient}">Modifier</a>
                </li>`;
        const separator = `<li><hr class="dropdown-divider"></li>`;
        const delete1 = `<li>
                    <a type="button" class="dropdown-item" data-action="delete" data-id="${full.idTypeClient}">Supprimer</a>
                </li>`;
  ...
  supprimer(id: any) {
    const modalRef = this.modalService.open(DeleteTypeclientModalComponent);
