...
export class TypegarantListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    public entityService: TypegarantService,
  ...
  ngOnInit(): void {
    this.datatableConfig = {
      ...
      columns: [
        {
          title: 'Code', data: 'codeTypeGarant', render: function (data, type, row) {
            return row.codeTypeGarant;
          }
        },{
          title: 'Libellé', data: 'libelleTypeGarant', render: function (data, type, row) {
            return row.libelleTypeGarant;
          }
        },
      ],
  ...
  renderEmetteurColumn(): void {
    if (this.datatableConfig.columns) {
      ...
      Emetteurs.render = (data: any, type: any, full: any) => {
        ...
        const show = `
                <li>
                    <a type="button" class="dropdown-item" data-action="view" data-id="${full.idTypeGarant}">Afficher</a>
                </li>`;
        const edit = `
                <li>
                    <a type="button" class="dropdown-item" data-action="edit" data-id="${full.idTypeGarant}">Modifier</a>
                </li>`;
        const separator = `<li><hr class="dropdown-divider"></li>`;
        const delete1 = `<li>
                    <a type="button" class="dropdown-item" data-action="delete" data-id="${full.idTypeGarant}">Supprimer</a>
                </li>`;
  ...
  supprimer(id: any) {
    const modalRef = this.modalService.open(DeleteTypegarantModalComponent);
