...
export class TypegarantAddEditComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.entityForm = this.fb.group(
      {
        id: [null],
        libelleTypeGarant: [null, Validators.required],
        codeTypeGarant: [null, Validators.required],
      }
    );
    // this.afficherCategorie();
    if (this.id) {
      console.log(this.id)
      this.pageInfo.updateTitle("Modification de type garant")
      ...
      this.pageInfo.updateTitle("Ajout de type garant")
    }
  }
  loadFormValues(entity: any)
  {
    this.entity = entity;
    this.entityForm.patchValue({libelleTypeGarant: entity.libelleTypeGarant});
    this.entityForm.patchValue({codeTypeGarant: entity.codeTypeGarant});
  }
  ...
  onSaveEntity() {
    ...
    const sb = this.saveEntity().pipe(
      ...
      finalize(() => {
        ...
        if (!this.isModal) this.router.navigate(['/app/standard/parametre/instrumentfinancier/typegarant']);
      })
    )
      .subscribe();
    this.subscriptions.push(sb);
