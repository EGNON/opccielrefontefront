...
export class BanqueAddEditComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private entityService: BanqueService,
  ...
  ngOnInit(): void {
    ...
    if (this.id) {
      console.log(this.id)
      this.pageInfo.updateTitle("Modification de banque")
      ...
      this.pageInfo.updateTitle("Ajout de banque")
    }
  }
  loadFormValues(entity: any)
  {
    this.entity = entity;
    this.entityForm.patchValue({nomBanque: entity.nomBanque});
    this.entityForm.patchValue({codeBanque: entity.codeBanque});
  }
  ...
  onSaveEntity() {
    ...
    const sb = this.saveEntity().pipe(
      ...
      finalize(() => {
        ...
        if (!this.isModal) this.router.navigate(['/app/standard/parametre/instrumentfinancier/banque']);
      })
    )
      .subscribe();
    this.subscriptions.push(sb);
