...
export class TypeobligationListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    public entityService: TypeobligationService,
  ...
  ngOnInit(): void {
    this.datatableConfig = {
      ...
      columns: [
        {
          title: 'Code', data: 'codeTypeObligation', render: function (data, type, row) {
            return row.codeTypeObligation;
          }
        },{
          title: 'Libellé', data: 'libelleTypeObligation', render: function (data, type, row) {
            return row.libelleTypeObligation;
          }
        },
      ],
  ...
  renderEmetteurColumn(): void {
    if (this.datatableConfig.columns) {
      ...
      Emetteurs.render = (data: any, type: any, full: any) => {
        ...
        const show = `
                <li>
                    <a type="button" class="dropdown-item" data-action="view" data-id="${full.idTypeObligation}">Afficher</a>
                </li>`;
        const edit = `
                <li>
                    <a type="button" class="dropdown-item" data-action="edit" data-id="${full.idTypeObligation}">Modifier</a>
                </li>`;
        const separator = `<li><hr class="dropdown-divider"></li>`;
        const delete1 = `<li>
                    <a type="button" class="dropdown-item" data-action="delete" data-id="${full.idTypeObligation}">Supprimer</a>
                </li>`;
  ...
  supprimer(id: any) {
    const modalRef = this.modalService.open(DeleteTypeobligationModalComponent);
