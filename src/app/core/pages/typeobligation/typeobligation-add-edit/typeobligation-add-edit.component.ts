...
export class TypeobligationAddEditComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private entityService: TypeobligationService,
  ...
  ngOnInit(): void {
    ...
    this.entityForm = this.fb.group(
      {
        id: [null],
        libelleTypeObligation: [null, Validators.required],
        codeTypeObligation: [null, Validators.required],
      }
    );
    // this.afficherCategorie();
    if (this.id) {
      console.log(this.id)
      this.pageInfo.updateTitle("Modification de type Obligation");
      ...
      this.pageInfo.updateTitle("Ajout de type Obligation");
    }
  }
  loadFormValues(entity: any)
  {
    this.entity = entity;
    this.entityForm.patchValue({libelleTypeObligation: entity.libelleTypeObligation});
    this.entityForm.patchValue({codeTypeObligation: entity.codeTypeObligation});
  }
  ...
  onSaveEntity() {
    ...
    const sb = this.saveEntity().pipe(
      ...
      finalize(() => {
        ...
        if (!this.isModal) this.router.navigate(['/app/standard/parametre/instrumentfinancier/typeobligation']);
      })
    )
      .subscribe();
    this.subscriptions.push(sb);
