...
export class NatureoperationListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    public entityService: NatureoperationService,
  ...
  ngOnInit(): void {
    this.datatableConfig = {
      ...
      columns: [
        {
          title: 'Code', data: 'codeNatureOperation', render: function (data, type, row) {
            return row.codeNatureOperation;
          }
        },{
          title: 'Libellé', data: 'libelleNatureOperation', render: function (data, type, row) {
  ...
  renderOperationColumn(): void {
    if (this.datatableConfig.columns) {
      ...
      Operations.render = (data: any, type: any, full: any) => {
        ...
        const show = `
                <li>
                    <a type="button" class="dropdown-item" data-action="view" data-id="${full.codeNatureOperation}">Afficher</a>
                </li>`;
        const edit = `
                <li>
                    <a type="button" class="dropdown-item" data-action="edit" data-id="${full.codeNatureOperation}">Modifier</a>
                </li>`;
        const separator = `<li><hr class="dropdown-divider"></li>`;
        const delete1 = `<li>
                    <a type="button" class="dropdown-item" data-action="delete" data-id="${full.codeNatureOperation}">Supprimer</a>
                </li>`;
  ...
  supprimer(id: any) {
    const modalRef = this.modalService.open(DeleteNatureoperationModalComponent);
