...
export class SecteurboursierAddEditComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private entityService: SecteurboursierService,
  ...
  ngOnInit(): void {
    ...
    this.entityForm = this.fb.group(
      {
        id: [null],
        libelleSecteurBoursier: [null, Validators.required],
      }
    );
    // this.afficherCategorie();
    if (this.id) {
      console.log(this.id)
      this.pageInfo.updateTitle("Modification de secteur boursier");
      ...
      this.pageInfo.updateTitle("Ajout de secteur boursier");
    }
  }
  loadFormValues(entity: any)
  {
    this.entity = entity;
    this.entityForm.patchValue({libelleSecteurBoursier: entity.libelleSecteurBoursier});
  }
  ...
  onSaveEntity() {
    ...
    const sb = this.saveEntity().pipe(
      ...
      finalize(() => {
        ...
        if (!this.isModal) this.router.navigate(['/app/standard/parametre/instrumentfinancier/secteurboursier']);
      })
    )
      .subscribe();
    this.subscriptions.push(sb);
