...
export class TypeemetteurListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    public entityService: TypeemetteurService,
  ...
  ngOnInit(): void {
    this.datatableConfig = {
      ...
      columns: [
        {
          title: 'Code', data: 'codeTypeEmetteur', render: function (data, type, row) {
            return row.codeTypeEmetteur;
          }
        },{
          title: 'Libellé', data: 'libelleTypeEmetteur', render: function (data, type, row) {
            return row.libelleTypeEmetteur;
          }
        },
      ],
  ...
  renderEmetteurColumn(): void {
    if (this.datatableConfig.columns) {
      ...
      Emetteurs.render = (data: any, type: any, full: any) => {
        ...
        const show = `
                <li>
                    <a type="button" class="dropdown-item" data-action="view" data-id="${full.codeTypeEmetteur}">Afficher</a>
                </li>`;
        const edit = `
                <li>
                    <a type="button" class="dropdown-item" data-action="edit" data-id="${full.codeTypeEmetteur}">Modifier</a>
                </li>`;
        const separator = `<li><hr class="dropdown-divider"></li>`;
        const delete1 = `<li>
                    <a type="button" class="dropdown-item" data-action="delete" data-id="${full.codeTypeEmetteur}">Supprimer</a>
                </li>`;
  ...
  supprimer(id: any) {
    const modalRef = this.modalService.open(DeleteTypeemetteurModalComponent);
