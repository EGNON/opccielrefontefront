...
export class TypeemetteurAddEditComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private entityService: TypeemetteurService,
  ...
  ngOnInit(): void {
    ...
    this.entityForm = this.fb.group(
      {
        id: [null],
        libelleTypeEmetteur: [null, Validators.required],
        codeTypeEmetteur: [null, Validators.required],
      }
    );
    // this.afficherCategorie();
    if (this.id) {
      console.log(this.id)
      this.pageInfo.updateTitle("Modification de type emetteur")
      ...
      this.pageInfo.updateTitle("Ajout de type emetteur")
    }
  }
  loadFormValues(entity: any)
  {
    this.entity = entity;
    this.entityForm.patchValue({libelleTypeEmetteur: entity.libelleTypeEmetteur});
    this.entityForm.patchValue({codeTypeEmetteur: entity.codeTypeEmetteur});
  }
  ...
  onSaveEntity() {
    ...
    const sb = this.saveEntity().pipe(
      ...
      finalize(() => {
        ...
        if (!this.isModal) this.router.navigate(['/app/standard/parametre/instrumentfinancier/typeemetteur']);
      })
    )
      .subscribe();
    this.subscriptions.push(sb);
