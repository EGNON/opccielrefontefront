...
export class FormuleAddEditComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private entityService: FormuleService,
  ...
  ngOnInit(): void {
    ...
    this.entityForm = this.fb.group(
      {
        id: [null],
        libelleFormule: [null, Validators.required],
        typeFormule: [null, Validators.required],
      }
    );
    this.afficherTypeformule()
    this.readOnly=false;
    // this.afficherCategorie();
    if (this.id) {
      this.readOnly=true;
      console.log(this.id)
      this.pageInfo.updateTitle("Modification de Formule")
      ...
      this.pageInfo.updateTitle("Ajout de Formule")
    }
  }
  afficherTypeformule(){
    this.typeFormuleService.afficherTous().subscribe(
      (data)=>{
        this.Typeformule$=data.data;
      }
    )
  }
  ...
  {
    this.entity = entity;
    this.entityForm.patchValue({libelleFormule: entity.libelleFormule});
    this.entityForm.patchValue({typeFormule: entity.typeFormule});
  }
  ...
  onSaveEntity() {
    ...
    const sb = this.saveEntity().pipe(
      ...
      finalize(() => {
        ...
        if (!this.isModal) this.router.navigate(['/app/standard/parametre/comptabilite/formule']);
      })
    )
      .subscribe();
    this.subscriptions.push(sb);
