...
export class PlaceListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    public entityService: PlaceService,
  ...
  ngOnInit(): void {
    this.datatableConfig = {
      ...
      columns: [
        {
          title: 'Code', data: 'codePlace', render: function (data, type, row) {
            return row.codePlace;
          }
        },{
          title: 'Libellé', data: 'libellePlace', render: function (data, type, row) {
            return row.libellePlace;
          }
        },
      ],
  ...
  renderEmetteurColumn(): void {
    if (this.datatableConfig.columns) {
      ...
      Emetteurs.render = (data: any, type: any, full: any) => {
        ...
        const show = `
                <li>
                    <a type="button" class="dropdown-item" data-action="view" data-id="${full.codePlace}">Afficher</a>
                </li>`;
        const edit = `
                <li>
                    <a type="button" class="dropdown-item" data-action="edit" data-id="${full.codePlace}">Modifier</a>
                </li>`;
        const separator = `<li><hr class="dropdown-divider"></li>`;
        const delete1 = `<li>
                    <a type="button" class="dropdown-item" data-action="delete" data-id="${full.codePlace}">Supprimer</a>
                </li>`;
  ...
  supprimer(id: any) {
    const modalRef = this.modalService.open(DeletePlaceModalComponent);
