...
export class ProduitListComponent implements OnInit, OnDestroy, AfterViewInit{
  ...
  afficherProduit(){
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        const sb = this.produitService.afficherTous(dataTablesParameters)
          .subscribe(resp => {
            callback(resp);
          });
  ...
  renderActionColumn(): void {
    if (this.datatableConfig.columns) {
      ...
      actions.render = (data: any, type: any, full: any) => {
        ...
        const show = `
                <li>
                    <a type="button" class="dropdown-item" data-action="view" data-id="${full.idProd}">Afficher</a>
                </li>`;
        const edit = `
                <li>
                    <a type="button" class="dropdown-item" data-action="edit" data-id="${full.idProd}">Modifier</a>
                </li>`;
        const separator = `<li><hr class="dropdown-divider"></li>`;
        const delete1 = `<li>
                    <a type="button" class="dropdown-item" data-action="delete" data-id="${full.idProd}">Supprimer</a>
                </li>`;
  ...
  supprimer(id: number) {
    const modalRef = this.modalService.open(DeleteProduitModalComponent);
    modalRef.componentInstance.id = id;
    modalRef.result.then(() => this.produitService.fetchRows(null), () => {});
  }
