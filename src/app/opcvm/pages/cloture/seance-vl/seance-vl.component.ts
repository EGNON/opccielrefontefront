...
import $ from "jquery";
import {SeancevlService} from "../../../services/seancevl.service";
...
export class SeanceVLComponent implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  ...
  constructor(
    ...
    private seanceVLService: SeancevlService,
  ...
  ngOnInit(): void {
    ...
    this.form = this.fb.group({
      ...
      VL: [this.currentSeance?.valeurLiquidative, Validators.required],
      dateDebut: [
    ...
    this.pageInfo.updateTitle("Clôture");
    this.afficherCloture()
    // this.afficherListe("l");
  }

  afficherCloture() {
    this.seanceVLService.afficherCloture(this.currentOpcvm?.idOpcvm).subscribe(
      (data)=>{
        this.seanceVL$=data.data
        console.log(this.seanceVL$)
      }
    );
  }
  verificationEcritureNiveau1De(){
    this.verifierEtape=true
    this.operationService.etape(9,
  ...
  verificationNiveau1() {
    ...
    this.operationService.afficherListeVerificationEritureNiveau1(param,"CHARGE").subscribe(
      (data)=>{
  ...
  clotureSeance(el: HTMLElement){
    ...
    this.seanceVLService.clotureSeance(
      this.authService.currentUserValue?.username,this.currentOpcvm?.idOpcvm).subscribe(
      (data)=>{
        this.valider=false
        if(data.data==="Clôture effectuée avec succès"){
          alert(data.data)
  ...
  afficherListe(prefix: string) {
    ...
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        typeOperationDto.codeTypeOperation="CHARGE"
