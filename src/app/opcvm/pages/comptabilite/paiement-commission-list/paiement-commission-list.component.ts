...
export class PaiementCommissionListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    private opComService: OperationCommissionService,) {
  }
  ...
  afficherListe(prefix: string) {
    let columns: any[] = [
      ...
      {
        title: 'COMMISSION', data: 'commission', render: function (data, type, full) {
          return '';
        }
      },
      {
        title: 'DATE SOLDE', data: 'dateSolde', render: function (data, type, row) {
          return moment(data).format('DD MMM YYYY à HH:mm:ss');
        },
      },
      {
        title: 'SOLDE', data: 'montant', render: function (data, type, full) {
          return full.montant;
        }
      },
      {
        title: 'MONTANT PAYE', data: 'montantCommission', render: function (data, type, full) {
          return full.montantCommission;
        }
      }
    ];
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if (prefix.toLowerCase() === "l") {
          console.log(param);
          const sb = this.opComService.afficherListeCommissions(param)
            .subscribe(resp => {
              callback(resp.data);
            });
