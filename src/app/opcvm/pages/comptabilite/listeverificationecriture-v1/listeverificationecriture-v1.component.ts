...
export class ListeverificationecritureV1Component implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  ...
  apercu() {
    ...
    console.log(param)
    this.operationService.apercuVerificationEritureNiveau(param,1,null).subscribe((blob) => {
  ...
  validationEcritureNiveau1(){
    ...
    this.operationService.validationEcritureNiveau1(this.idOperation,
      this.authService.currentUserValue?.username,null,null,this.currentOpcvm?.idOpcvm).subscribe(
      (data)=>{
