...
export class ProfilcommissionsousrachListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.datatableConfig = {
      ...
      columns: [
        {
          title: 'Type', data: 'typeCommission', render: function (data, type, row) {
            return row.typeCommission;
          }
        },
        {
          title: 'Code', data: 'codeProfil', render: function (data, type, row) {
            return row.codeProfil;
          }
        },
        {
          title: 'Libellé', data: 'libelleProfil', render: function (data, type, row) {
            return row.libelleProfil;
          }
        },
        {
          title: 'S?', data: 'standard', render: function (data, type, row) {
            //return row.standard;
            return `<div class="badge ${row.standard ? 'badge-success' : 'badge-info'} fw-bold">${row.standard ? 'Oui' : 'Non'}</div>`;
          }
        },
      ],
