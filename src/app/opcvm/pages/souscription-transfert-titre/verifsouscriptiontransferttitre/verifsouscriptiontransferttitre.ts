...
@Component({
  selector: 'app-verifsouscriptiontransferttitre',
  standalone: false,
  templateUrl: './verifsouscriptiontransferttitre.html',
  styleUrl: './verifsouscriptiontransferttitre.scss'
})
export class Verifsouscriptiontransferttitre implements OnInit, OnDestroy {
    ...
    this.seanceOpcvmService.afficherSeanceEnCours(this.localStore.getData("currentOpcvm").idOpcvm)
      .subscribe(
        (data)=>{
          ...
          this.afficherVerificationSouscriptionTransfertTitre()
        }
      )

  }
  afficherVerificationSouscriptionTransfertTitre()
  {
  ...
  verifiSOuscriptionTRansfertTitre()
  {
    this.verifierApercu=true
    this.entityService.verifSouscriptionTransfertTitre(
     ...
     .subscribe((blob) => {
        ...
        a.href = url;
        a.download = 'verif_souscription_transfert_titre.pdf';
        a.click();
      });
  }
  ngOnDestroy(): void {
