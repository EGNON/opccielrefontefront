...
@Component({
  selector: 'app-point-actif-net-part-vl',
  standalone: false,
  templateUrl: './point-actif-net-part-vl.html',
  styleUrl: './point-actif-net-part-vl.scss'
})
export class PointActifNetPartVl implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  form: FormGroup;
...
telechargerDocuments(data:any){
  ...
  actualiser() {
    let idOpcvm = this.currentOpcvm?.idOpcvm;
        let param = {
          ...
          console.log(param);
          const sb = this.libService.pointActifNetPartVlListe(param)
            .subscribe(resp => {
              console.log("Retour releve === ", resp.data);
              this.pointActifNetPartVl$=resp.data
            });
          this.subscriptions.push(sb);
  ...
  exportExcel() {
      ...
      const headers = ['ID SEANCE','DATE SEANCE','ACTIF NET','NBRE DE PART EN CIRCULATION','VL'];
      let idOpcvm = this.currentOpcvm?.idOpcvm;
      let param = {
        idOpcvm: idOpcvm
      };
      ...
      this.libService.pointActifNetPartVlListe(param).subscribe(
        (data)=>{
          this.allData=data.data;
          const exportData = this.allData.map(item => ({
            'ID SEANCE': item.idSeance,
            'DATE SEANCE':moment(item.dateValeur).format('DD/MM/YYYY'),
            'ACTIF NET': item.actifNet,
            'NBRE DE PART EN CIRCULATION': item.npc,
            'VL': item.vl
          }));
          ...
          saveAs(blob, 'point-actif-net-part-vl.xlsx');
          this.export=false
        }
      )
