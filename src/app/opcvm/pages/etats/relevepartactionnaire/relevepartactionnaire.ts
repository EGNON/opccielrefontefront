...
@Component({
  selector: 'app-relevepartactionnaire',
  standalone: false,
  templateUrl: './relevepartactionnaire.html',
  styleUrl: './relevepartactionnaire.scss'
})
export class Relevepartactionnaire implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  form: FormGroup;
...
afficherActionnaire(){
  ...
  telecharger() {
    ...
    //.subscribe
    const sb = this.libService.relevePartActionnaireEtat(param)
      .pipe(
        catchError((err) => {
      ...
      .subscribe((blob) => {
        ...
        a.href = url;
        a.download = 'releve_part_actionnaire.pdf';
        a.click();
      });
    this.subscriptions.push(sb);
  }
