...
@Component({
  selector: 'app-souscriptionglobal',
  standalone: false,
  templateUrl: './souscriptionglobal.html',
  styleUrl: './souscriptionglobal.scss'
})
export class Souscriptionglobal implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  form: FormGroup;
...
afficherdistributeur(){
  ...
  afficherListe(prefix: string) {
    const self = this;
    if(this.form.value.distributeur==null){
     ...
     let columns: any[] = [
      ...
      // },
      {
        title: 'MT. DEPOT', data: 'montantDepose', render: function (data, type, row) {
          return row.montantDepose;
        },
      },
      {
      ...
      {
        title: 'S/R A LIQUIDER', data: 'montantSousALiquider', render: function (data, type, row) {
    ...
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if (prefix.toLowerCase() === "l") {
          ...
          console.log(param);
          const sb = this.libService.souscriptionglobal(param)
            .subscribe(resp => {
 ...
 afficherListeVide(prefix: string) {
  ...
  exportExcel() {
    ...
    const headers = ['DISTRIBUTEUR','DATE OPER.','TYPE',
      'MT. DEPOT','PART','S/R A LIQUIDER','COMMISSION','RETROCESSION'];
    let idOpcvm = this.currentOpcvm?.idOpcvm;
    let param = {
      idOpcvm: idOpcvm,
      ...this.form.value,
    ...
    this.libService.souscriptionglobalListe(param).subscribe(
      (data)=>{
        ...
        const exportData = this.allData.map(item => ({
          ...
          'MT. DEPOT': item.montantDepose,
          'PART': item.nombrePartSousRachat,
          'S/R A LIQUIDER': item.montantSousALiquider,
        ...
        saveAs(blob, 'souscription_global.xlsx');
        this.export=false
      }
    )
  ...
  telecharger() {
    ...
    //.subscribe
    console.log(param);
    const sb = this.libService.souscriptionglobalEtat(param)
      .pipe(
        catchError((err) => {
       ...
       .subscribe((blob) => {
        ...
        a.href = url;
        a.download = 'souscription_global.pdf';
        a.click();
      });
    this.subscriptions.push(sb);
  }
