...
@Component({
  selector: 'app-relevepartfcp',
  standalone: false,
  templateUrl: './relevepartfcp.html',
  styleUrl: './relevepartfcp.scss'
})
export class Relevepartfcp implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  form: FormGroup;
    ...
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if (prefix.toLowerCase() === "l") {
          ...
          console.log(param);
          const sb = this.libService.relevePartFCP(param)
            .subscribe(resp => {
  ...
  exportExcel() {
    ...
    this.libService.relevePartFCPListe(param).subscribe(
      (data)=>{
        ...
        saveAs(blob, 'relevePartFCP.xlsx');
        this.export=false
      }
    )
  ...
  telecharger() {
    ...
    //.subscribe
    const sb = this.libService.relevePartFCPEtat(param)
      .pipe(
        catchError((err) => {
      ...
      .subscribe((blob) => {
        ...
        a.href = url;
        a.download = 'releve_part_fcp.pdf';
        a.click();
      });
    this.subscriptions.push(sb);
  }
