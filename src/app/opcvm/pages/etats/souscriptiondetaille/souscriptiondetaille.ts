...
@Component({
  selector: 'app-souscriptiondetaille',
  standalone: false,
  templateUrl: './souscriptiondetaille.html',
  styleUrl: './souscriptiondetaille.scss'
})
export class Souscriptiondetaille implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  form: FormGroup;
...
afficherdistributeur(){
    ...
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if (prefix.toLowerCase() === "l") {
          ...
          console.log(param);
          const sb = this.libService.souscriptionDetaille(param)
            .subscribe(resp => {
 ...
 afficherListeVide(prefix: string) {
  ...
  exportExcel() {
    ...
    this.libService.souscriptionDetailleListe(param).subscribe(
      (data)=>{
        ...
        saveAs(blob, 'souscription_detaillé.xlsx');
        this.export=false
      }
    )
  ...
  telecharger() {
    ...
    //.subscribe
    console.log(param);
    const sb = this.libService.souscriptionDetailleEtat(param)
      .pipe(
        catchError((err) => {
       ...
       .subscribe((blob) => {
        ...
        a.href = url;
        a.download = 'souscription_detaille.pdf';
        a.click();
      });
    this.subscriptions.push(sb);
  }
