...
@Component({
  selector: 'app-rachatglobal',
  standalone: false,
  templateUrl: './rachatglobal.html',
  styleUrl: './rachatglobal.scss'
})
export class Rachatglobal implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  form: FormGroup;
  ...
  afficherListe(prefix: string) {
    ...
    if(this.form.value.distributeur==null){
     ...
     let columns: any[] = [
      ...
      {
        title: 'MONTANT. RACHAT', data: 'montantSousALiquider', render: function (data, type, row) {
    ...
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if (prefix.toLowerCase() === "l") {
          ...
          console.log(param);
          const sb = this.libService.rachatglobal(param)
            .subscribe(resp => {
 ...
 afficherListeVide(prefix: string) {
  ...
  exportExcel() {
    ...
    const headers = ['DISTRIBUTEUR','DATE OPER.','TYPE',
      'PART','MONTANT. RACHAT','COMMISSION','RETROCESSION'];
    let idOpcvm = this.currentOpcvm?.idOpcvm;
    let param = {
      idOpcvm: idOpcvm,
      ...this.form.value,
    ...
    this.libService.rachatglobalListe(param).subscribe(
      (data)=>{
        ...
        const exportData = this.allData.map(item => ({
          ...
          'MONTANT. RACHAT': item.montantSousALiquider,
        ...
        saveAs(blob, 'rachat_global.xlsx');
        this.export=false
      }
    )
  ...
  telecharger() {
    ...
    //.subscribe
    console.log(param);
    const sb = this.libService.rachatglobalEtat(param)
      .pipe(
        catchError((err) => {
