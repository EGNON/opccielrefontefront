import { AfterContentInit, AfterViewInit, ChangeDetectorRef, Component, OnDestroy, OnInit, Renderer2, ViewChild, ViewContainerRef } from '@angular/core';
...
@Component({
  selector: 'app-relachementordre',
  standalone: false,
  templateUrl: './relachementordre.html',
  styleUrl: './relachementordre.scss'
})
export class Relachementordre implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  form: FormGroup;
  listeAvisForm: any
    ...
    let columns: any[] = [
      {
        ...
        render: (data: any, type: any, row: any) => {
          return `<input type="checkbox" class="row-checkbox" data-id="${row.idOperation}">`;
        }
      },
      {
        sortable: false,
    ...
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if (prefix.toLowerCase() === "l") {
          ...
          console.log(param);
          const sb = this.librairieService.listeOrdreDeBourse(param)
            .subscribe(resp => {
              callback(resp.data);
      ...
      createdRow: function (row, data: any, dataIndex) {
        $(row).find('.row-checkbox').on('change', (e) => {
          ...
          console.log("IDs sélectionnés :", self.selectedIds);
        });
  ...
  saveEntity() {
    if(this.selectedIds.length===0){
      ...
      console.log("act1",param)
      
      this.librairieService.enregistrerordrerelache(param).subscribe(
        (data)=>{
