...
export class Verificationextourneniveau2modalvdeComponent implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.pageInfo.updateTitle("Vérification des écritures d'extourne des VDE Niveau 2");
    this.dtOptions = {
      // dom: 'Bfrtip',
  ...
  apercu() {
    ...
    this.operationService.apercuVerificationEritureNiveau(param,2,"DE").pipe(
      ...
      finalize(() => {
        ...
        a.href = url;
        a.download = 'verification__ecriture_extourne_vde_niveau2.pdf';
        a.click();
      });
    // this.downloading=false
  }
  apercuFinal() {
    ...
    this.operationService.apercuVerificationEritureNiveau(param,2,"DE").pipe(
      ...
      finalize(() => {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = 'verification_ecriture_extourne_vde_niveau2_finale.pdf';
        a.click();
      });
    // this.downloading=false
  }
  validationEcritureNiveau1(){
    ...
    this.operationService.validationEcritureNiveau2(this.idOperation,
      this.authService.currentUserValue?.username,"DE","Extourne",this.currentOpcvm?.idOpcvm).subscribe(
      (data)=>{
  ...
  afficherListe(prefix: string) {
    ...
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.typeOperation.codeTypeOperation="DE"
        if (prefix.toLowerCase() === "l") {
          param = {
            ...
            dateFin: new Date(param.dateFin.year, param.dateFin.month-1, param.dateFin.day+1),
            estVerifie1:true,
            estVerifie2:estVerifie1,
            typeOperationDto:this.typeOperation,
            codeTypeOperation:"DE"
          };
          console.log(param);
