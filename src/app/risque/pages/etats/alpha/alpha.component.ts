...
export class AlphaComponent implements OnInit, OnDestroy, AfterViewInit{
      ...
      columns: [
        {
          title: "DATE .", data: 'dateFermeture', render: function (data: any, type: any, row: any) {
            ...
            //return daterecup[2]+"/"+libelleMois+"/"+daterecup[0];
        ...
        {
          title: "Indice de référence", data: 'indiceReference', render: function (data: any, type: any, row: any) {
            return row.indiceReference;
          }
        },
        {
          title: "Alpha", data: 'alpha', render: function (data: any, type: any, row: any) {
            return row.alpha;
          }
        }
      ],
  ...
  loadData(submitted = false) {
    ...
    //   this.dtTrigger.next(null);
    // });
    this.reportingsService.alpha(this.idOpcvm,
                                this.form.value.anneeDebut,
                                this.form.value.anneeFin)
      .pipe(first())
      .subscribe(data => {
        this.dataTable = this.initDatatable("datatable_Alpha");
