...
export class BetaComponent implements OnInit, OnDestroy, AfterViewInit{
  ...
  ngOnInit(): void {
    this.form = this.fb.group({
      ...
      beta: [null],
      volatiliteAnnualiseeOpcvm: [null],
      volatiliteAnnualiseeBenchMark: [null],
    });
    this.afficherOpcvm()
  ...
  initDatatable(tableId: any) {
    ...
    const datatable = $(`#${tableId}`).DataTable({
      ...
      columns: [
        ...
        /*,
        {
          title: "Volatilité annualisée de l'OPCVM", data: 'volatiliteAnnualiseeOpcvm', render: function (data: any, type: any, row: any) {
            return row.volatiliteAnnualiseeOpcvm;
          }
        },
        {
          title: "Volatilité annualisée du BenchMark", data: 'volatiliteAnnualiseeBenchMark', render: function (data: any, type: any, row: any) {
            return row.volatiliteAnnualiseeBenchMark;
          }
        }*/
      ],
  ...
  loadData(submitted = false) {
    ...
    this.reportingsService.beta(this.idOpcvm,
      dates)
      .pipe(first())
      .subscribe(data => {
       // console.log(data)
        ...
        {
          //console.log("beta",this.beta[0].beta)
          this.form.patchValue({beta:this.beta[0].beta})
          this.form.patchValue({volatiliteAnnualiseeBenchMark:this.beta[0].volatiliteAnnualiseeBenchMark})
          this.form.patchValue({volatiliteAnnualiseeOpcvm:this.beta[0].volatiliteAnnualiseeOpcvm})
        }
        this.dataTable = this.initDatatable("datatable_beta");
